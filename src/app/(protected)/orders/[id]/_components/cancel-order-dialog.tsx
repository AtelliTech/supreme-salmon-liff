...
export const CancelOrderDialog = forwardRef<
  HTMLDialogElement,
  { userId: string; orderNumber: string }
>(function CancelOrderDialog({ userId, orderNumber }, ref) {
  ...
  const mutation = useMutation({
    mutationFn: () => api.cancelOrder(userId, orderNumber, {}).json(),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [userId, "orders"] });
      close();
        ...
        <div className="w-full max-w-sm overflow-hidden rounded-2xl bg-white shadow-2xl">
          ...
          <div className="grid grid-cols-2 gap-3 p-4">
            ...
            <button
              ...
              className="w-full rounded-xl bg-salmon-500 px-4 py-3 font-semibold text-sm text-white shadow-md transition-colors hover:bg-salmon-600 disabled:opacity-60"
            >
              {mutation.isPending ? "取消中..." : "取消訂單"}
            </button>
          </div>
        </div>
      </div>
